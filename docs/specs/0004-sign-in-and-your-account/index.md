...
11. **Finish the attempt limiting.** Step 9 guarded the three actions that existed when it was written: sign in, sign up, and password reset. Five more need it, and they fall into two groups.
    ...
    - **Code submitters**: `verifyEmailCode`, `setNewPassword`, and `changePassword`. None of these was limited before this step, so nothing in this app counted wrong guesses at a six digit code. Whether InsForge throttles `exchangeResetPasswordToken` on its own side has since been **confirmed on the test project, and it does not**: ten wrong codes inside a minute were all accepted for checking, and the eleventh was refused by this app and nothing else. Mail sending is limited on their side; guessing is not. A platform side limit would not have removed the need for one here anyway, because a limit you did not configure is a limit that can change without telling you.
...
- [x] Build step 11: the five actions named here are now behind the limiter, so AC-8 holds. The two signed in ones are keyed on the account alone rather than on the pair of account and source, because keying on the pair hands a fresh allowance to every new source and defeats the point.
- [x] Confirmed on the test project, and the answer cuts both ways. Sending mail **is** limited on InsForge's side: `resendVerificationEmail` returns a sixty second per address cooldown and then a per IP refusal ("Too many send email verification requests from this IP. Please try again in 15 minutes"), and `sendResetPasswordEmail` returns `429 TOO_MANY_REQUESTS` per IP. Exchanging a code is **not**: `exchangeResetPasswordToken` accepted ten wrong codes inside a minute without complaint, and the eleventh was stopped by this app's own limiter and nothing else. So the guess window really was unprotected before step 11.
