...
5. [x] Configure the alert rule so a new issue emails you and a recurrence does not, satisfies **AC-3**. Done on 2026-09-01. The note written here on 31 August was wrong: the project has had an issue alert rule since it was created, and that rule fired for both smoke test reports. What it did carry was a second condition, "Sentry marks an existing issue as high priority", which is not what this criterion asks for; that condition was removed, so the rule now fires on a new high priority issue only. The email had been arriving all along and being filtered as spam, which is indistinguishable from a missing rule until you read the rule's trigger history. Whitelist the sender: a spam filter is the one link in this chain that neither Sentry nor this repo can see.
