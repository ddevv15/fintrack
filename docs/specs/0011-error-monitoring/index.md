...
5. [ ] Configure the alert rule so a new issue emails you and a recurrence does not, satisfies **AC-3**. **The rule is right now; delivery is unconfirmed.** The note written here on 31 August was wrong: the project has had an issue alert rule since it was created, and that rule fired for both smoke test reports. What it did carry was a second condition, "Sentry marks an existing issue as high priority", which is not what this criterion asks for; that condition was removed on 2026-09-01, so the rule now fires on a new high priority issue only. It routes to email, and the account accepts issue alert email at a verified address with no per project override. A third, deliberately distinct error on 2026-09-01 produced a new issue and fired the rule. All that remains is whether the mail reaches the inbox, which is why this stays open.
