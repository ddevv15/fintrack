...
> **Build progress** (updated by `/develop`, 2026-08-19). Boxes 2 to 7 and 11 to 13 are done, applied, and verified against the live database. Boxes 8 to 10 are written but unrun, and box 1 changed shape: no separate backend branch was created (the project was empty with zero users, so the branch would have protected nothing), and the two test accounts are blocked because the project requires email verification while SMTP is off. Both deviations were agreed during the build and are listed in Follow-up.
...
1. [ ] Create a non production InsForge branch and two test accounts, add the six test only environment values to `.env.example` and an optional schema in `lib/env.ts`, and confirm `.env*.local` stays ignored. Prerequisite for **AC-7**, **AC-10**.
...
8. [ ] Add `tests/integration/schema-drift.test.ts`: read one real row from each of the three tables on the test branch and assert its Zod schema parses it, failing when the live schema and the schema file disagree. Satisfies **AC-10**.
9. [ ] Add `tests/integration/row-level-security.test.ts`: sign in as both test accounts and run the cross account select, insert, update, and delete attempts, plus the category restrict, kind mismatch, and constraint refusals. Satisfies **AC-3**, **AC-5**, **AC-6**, **AC-7**.
10. [ ] Add a Postgres exactness check to the integration test: insert a set of awkward amounts, sum them in SQL, and assert the exact cents, so the money guarantee is tested and not merely declared. Satisfies **AC-2**.
...
- [ ] **Email verification is required but SMTP is off**, so no signup can complete on the project today. This blocks the two test accounts, and it will block feature 5. You are setting up SMTP; once it works, sign up and verify the two test addresses once and the integration suite runs on its own from then on.
